...
describe('Guided Workflow Prompts', () => {
  ...
  beforeEach(async () => {
    ...
    await fs.writeFile(join(tempDir, 'tests/index.test.js'), 'test("basic", () => {});');
  });
  ...
  describe('analyze-and-recommend prompt', () => {
    it('should generate comprehensive analysis and recommendation prompt', async () => {
      const messages = await generateTechnicalWriterPrompts('analyze-and-recommend', tempDir, {
      ...
      expect(messages[0].content.text).toContain('Execute a complete repository analysis');
      ...
      expect(messages[0].content.text).toContain('Preferences: performance and ease of use');
    ...
    it('should use default values when optional parameters are not provided', async () => {
      ...
      expect(messages[0].content.text).toContain('Analysis Depth: standard');
      expect(messages[0].content.text).toContain('balanced approach with good community support');
    });
  ...
  describe('setup-documentation prompt', () => {
    it('should generate comprehensive documentation setup prompt', async () => {
      const messages = await generateTechnicalWriterPrompts('setup-documentation', tempDir, {
        ssg_type: 'docusaurus',
      ...
      expect(messages).toHaveLength(1);
      expect(messages[0]).toHaveProperty('role', 'user');
      expect(messages[0].content.text).toContain('Create a comprehensive documentation structure');
      ...
      expect(messages[0].content.text).toContain('Diataxis Framework Implementation');
      expect(messages[0].content.text).toContain('Tutorials: Learning-oriented content');
      expect(messages[0].content.text).toContain('How-to Guides: Problem-solving content');
      expect(messages[0].content.text).toContain('Reference: Information-oriented content');
      expect(messages[0].content.text).toContain('Explanations: Understanding-oriented content');
    ...
    it('should handle minimal configuration', async () => {
      ...
      expect(messages[0].content.text).toContain('SSG Type: recommended based on project analysis');
    ...
    it('should include current documentation gaps', async () => {
      ...
      expect(messages[0].content.text).toContain('production-ready documentation system');
    });
  });

  describe('troubleshoot-deployment prompt', () => {
    it('should generate comprehensive troubleshooting prompt', async () => {
      const messages = await generateTechnicalWriterPrompts('troubleshoot-deployment', tempDir, {
      ...
      expect(messages).toHaveLength(1);
      expect(messages[0]).toHaveProperty('role', 'user');
      expect(messages[0].content.text).toContain('Diagnose and fix GitHub Pages deployment issues');
      expect(messages[0].content.text).toContain('Repository: owner/repo');
      expect(messages[0].content.text).toContain('Expected URL: https://owner.github.io/repo');
    ...
    it('should use default values for optional parameters', async () => {
      ...
      expect(messages[0].content.text).toContain('Expected URL: GitHub Pages URL');
      expect(messages[0].content.text).toContain(
  ...
  describe('Prompt content validation', () => {
    it('should generate prompts with consistent structure', async () => {
      ...
      for (const promptType of promptTypes) {
        const args = promptType === 'troubleshoot-deployment' ? { repository: 'test/repo' } : {};

        const messages = await generateTechnicalWriterPrompts(promptType, tempDir, args);
    ...
    it('should include project-specific information in all prompts', async () => {
      ...
      for (const promptType of promptTypes) {
        const messages = await generateTechnicalWriterPrompts(promptType, tempDir, {});
