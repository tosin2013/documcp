...
describe('generate-technical-writer-prompts', () => {
  ...
  describe('Project Context Analysis', () => {
    it('should detect Node.js project with React', async () => {
      ...
      expect(result.isError).toBe(false);
      expect(result.generation.contextSummary.projectContext.projectType).toBe('web_application');
      expect(result.generation.contextSummary.projectContext.frameworks).toContain('React');
      expect(result.generation.contextSummary.projectContext.languages).toContain('TypeScript');
      expect(result.generation.contextSummary.projectContext.languages).toContain('JavaScript');
    });

    it('should detect Python project', async () => {
      await fs.writeFile(join(testProjectPath, 'main.py'), 'print("Hello, World!")');
      await fs.writeFile(join(testProjectPath, 'requirements.txt'), 'flask==2.0.0');
    ...
    it('should detect test files', async () => {
      await fs.writeFile(join(testProjectPath, 'test.js'), 'describe("test", () => {})');
      await fs.writeFile(join(testProjectPath, 'package.json'), JSON.stringify({ name: 'test' }));
      ...
      expect(result.isError).toBe(false);
      expect(result.generation.contextSummary.projectContext.hasTests).toBe(true);
    });
  });
  ...
  describe('Documentation Context Analysis', () => {
    it('should detect existing README', async () => {
      ...
      expect(result.isError).toBe(false);
      expect(result.generation.contextSummary.documentationContext.readmeExists).toBe(true);
    });
    ...
    it('should handle missing README', async () => {
      ...
      expect(result.isError).toBe(false);
      expect(result.generation.contextSummary.documentationContext.readmeExists).toBe(false);
    });
  });
  ...
  describe('Prompt Generation', () => {
    ...
    it('should generate style improvement prompts for low health scores', async () => {
      await fs.writeFile(join(testProjectPath, 'README.md'), '# Test\nBad readme');
  ...
  describe('Recommendations and Next Steps', () => {
    ...
    it('should generate integration recommendations', async () => {
      ...
      expect(result.generation.recommendations[0]).toContain('analyze_repository');
    });
  ...
  describe('Error Handling', () => {
    it('should handle non-existent project path gracefully', async () => {
      ...
      expect(result.generation.contextSummary.projectContext.projectType).toBe('unknown');
    });
    ...
    it('should handle invalid context sources', async () => {
      ...
      expect(result.isError).toBe(true);
      expect(result.content[0].text).toContain('Error generating technical writer prompts');
    });
