...
`);
      ...
      const conductCheck = data.healthReport.components.communityHealth.details
        .find((d: any) => d.check === 'Code of Conduct linked');
...
`);
      ...
      const contributingCheck = data.healthReport.components.communityHealth.details
        .find((d: any) => d.check === 'Contributing guidelines visible');
...
`);
      ...
      const securityCheck = data.healthReport.components.communityHealth.details
        .find((d: any) => d.check === 'Security policy linked');
...
`);
      ...
      const supportCheck = data.healthReport.components.communityHealth.details
        .find((d: any) => d.check === 'Support channels provided');
...
`);
      ...
      const spacingCheck = data.healthReport.components.accessibility.details
        .find((d: any) => d.check === 'Scannable structure with proper spacing');
      expect(spacingCheck.passed).toBe(true);
    });

    test('should detect heading hierarchy', async () => {
      const readmePath = join(testDir, 'README.md');
      await writeFile(readmePath, `# Main Title
...
`);
      ...
      const headingCheck = data.healthReport.components.accessibility.details
        .find((d: any) => d.check === 'Clear heading hierarchy');
...
`);
      ...
      const altTextCheck = data.healthReport.components.accessibility.details
        .find((d: any) => d.check === 'Alt text for images');
...
`);
      ...
      const inclusiveCheck = data.healthReport.components.accessibility.details
        .find((d: any) => d.check === 'Inclusive language');
...
`);
      ...
      const quickStartCheck = data.healthReport.components.onboarding.details
        .find((d: any) => d.check === 'Quick start section');
...
`);
      ...
      const prereqCheck = data.healthReport.components.onboarding.details
        .find((d: any) => d.check === 'Prerequisites clearly listed');
...
`);
      ...
      const firstContribCheck = data.healthReport.components.onboarding.details
        .find((d: any) => d.check === 'First contribution guide');
...
`);
      ...
      const goodFirstCheck = data.healthReport.components.onboarding.details
        .find((d: any) => d.check === 'Good first issues mentioned');
  ...
  describe('Content Quality Evaluation', () => {
    test('should evaluate adequate content length', async () => {
      ...
      const lengthCheck = data.healthReport.components.contentQuality.details
        .find((d: any) => d.check === 'Adequate content length');
...
`);
      ...
      const codeCheck = data.healthReport.components.contentQuality.details
        .find((d: any) => d.check === 'Code examples provided');
...
`);
      ...
      const linksCheck = data.healthReport.components.contentQuality.details
        .find((d: any) => d.check === 'External links present');
...
`;
      ...
      const clarityCheck = data.healthReport.components.contentQuality.details
        .find((d: any) => d.check === 'Project description clarity');
  ...
  describe('Grade Calculation', () => {
    test('should assign grade A for 90%+ score', async () => {
      ...
      await writeFile(readmePath, `# Excellent Project
...
`);
  ...
  describe('Recommendations and Critical Issues', () => {
    test('should identify critical issues for low-scoring components', async () => {
      ...
      expect(data.healthReport.criticalIssues.some((issue: string) => issue.includes('Critical:'))).toBe(true);
    });
