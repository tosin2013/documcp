...
describe('EnhancedMemoryManager', () => {
  ...
  beforeEach(async () => {
    ...
    tempDir = path.join(
      os.tmpdir(),
      `enhanced-memory-test-${Date.now()}-${Math.random().toString(36).substr(2, 9)}`,
    );
  ...
  describe('Intelligent Analysis', () => {
    test('should provide intelligent analysis with patterns and predictions', async () => {
      ...
      expect(typeof intelligentAnalysis.learningData.similarProjects).toBe('number');
      expect(typeof intelligentAnalysis.learningData.confidenceLevel).toBe('number');
      expect(['low', 'medium', 'high']).toContain(intelligentAnalysis.learningData.dataQuality);

      // Check prediction structure
      if (intelligentAnalysis.predictions.length > 0) {
        const prediction = intelligentAnalysis.predictions[0];
        expect(['success_rate', 'optimal_ssg', 'potential_issues']).toContain(prediction.type);
    ...
    test('should adapt analysis based on historical patterns', async () => {
      ...
      const analysis = await enhancedManager.getIntelligentAnalysis('/test/project', analysisData);
  ...
  describe('Performance and Optimization', () => {
    ...
    test('should provide optimization insights', async () => {
      ...
      expect(['nascent', 'developing', 'mature']).toContain(stats.combined.systemMaturity);
    });
  });
