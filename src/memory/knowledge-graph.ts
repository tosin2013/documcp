...
export class KnowledgeGraph {
  ...
  private async computePatternRelationships(): Promise<void> {
    const successfulOutcomes = Array.from(this.nodes.values())
      .filter(node => node.type === 'outcome' && node.properties.status === 'success');
    ...
    for (const outcome of successfulOutcomes) {
      ...
      const incomingEdges = Array.from(this.edges.values())
        .filter(edge => edge.target === outcome.id);
  ...
  private calculateSuccessRate(techId: string): number {
    ...
    const outcomes = Array.from(this.nodes.values())
      .filter(node =>
        node.type === 'outcome' &&
        node.properties.ssg === tech
      );
  ...
  private generateReasoning(path: GraphPath): string[] {
    ...
    for (let i = 0; i < path.edges.length; i++) {
      ...
      switch (edge.type) {
        case 'similar_to':
          reasoning.push(`Similar to ${sourceNode.label} (${(edge.confidence * 100).toFixed(0)}% similarity)`);
          break;
        case 'recommends':
          reasoning.push(`Successfully used ${targetNode.label} (score: ${edge.weight.toFixed(1)})`);
          break;
        case 'results_in':
  ...
  private calculatePathConfidence(path: GraphPath, features: any): number {
    ...
    const avgAge = path.nodes.reduce((sum, node) => {
  ...
  async loadFromMemory(): Promise<void> {
    try {
      ...
      if (graphMemories.length > 0) {
        const latestGraph = graphMemories
  ...
  async getStatistics(): Promise<{
    ...
    const averageConnectivity = connectivityCounts.length > 0
      ? connectivityCounts.reduce((sum, { connections }) => sum + connections, 0) / connectivityCounts.length
      : 0;
