...
const AnalyzeReadmeInputSchema = z.object({
  ...
  target_audience: z.enum(['community_contributors', 'enterprise_users', 'developers', 'general']).optional().default('community_contributors'),
...
export async function analyzeReadme(input: Partial<AnalyzeReadmeInput>): Promise<MCPToolResponse<{ analysis: ReadmeAnalysis; nextSteps: string[] }>> {
...
function calculateOverallScore(
  ...
  score += lengthAnalysis.exceedsTarget ? 
    Math.max(0, 25 - (lengthAnalysis.reductionNeeded / 10)) : 25;
...
function generateRecommendations(
  ...
  if (targetAudience === 'community_contributors') {
    recommendations.push('👥 Focus on community onboarding: clear contributing guidelines and issue templates');
  ...
  if (optimizationLevel === 'aggressive') {
    recommendations.push('⚡ Consider moving detailed documentation to separate files (docs/ directory)');
