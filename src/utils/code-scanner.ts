...
import { MultiLanguageCodeScanner, LanguageParseResult } from './language-parsers-simple.js';

export interface CodeElement {
  name: string;
  type: 'function' | 'class' | 'interface' | 'type' | 'enum' | 'variable' | 'export' | 'import';
  filePath: string;
  line: number;
  column: number;
  exported: boolean;
...
export class CodeScanner {
  ...
  async analyzeRepository(): Promise<CodeAnalysisResult> {
    ...
    result.supportedLanguages = this.multiLanguageScanner.getSupportedExtensions();
  ...
  private async analyzePackageJson(result: CodeAnalysisResult): Promise<void> {
    try {
      ...
      if (allDeps.some((dep) => dep.startsWith('@npm/'))) result.packageManagers.push('npm');
      if (allDeps.some((dep) => dep.includes('yarn'))) result.packageManagers.push('yarn');
      if (allDeps.some((dep) => dep.includes('pnpm'))) result.packageManagers.push('pnpm');
  ...
  private async analyzeFile(filePath: string, result: CodeAnalysisResult): Promise<void> {
    try {
      ...
      if (this.multiLanguageScanner.getSupportedExtensions().includes(extension)) {
        const parseResult = await this.multiLanguageScanner.parseFile(content, filePath);
  ...
  private async analyzeTypeScriptFile(
    ...
    try {
      ...
      throw new Error(`TypeScript parsing failed for ${relativePath}: ${error}`);
    }
  }

  private mergeParseResults(parseResult: LanguageParseResult, result: CodeAnalysisResult): void {
      ...
      case 'ClassDeclaration':
        if (node.id?.name) {
          result.classes.push({
            ...
            hasJSDoc: this.hasJSDoc(node),
            jsDocDescription: this.getJSDocDescription(node),
            decorators: node.decorators?.map((d: any) => d.expression?.name || 'decorator') || [],
          });
        }
        break;
  ...
  private findAPIEndpoints(content: string, result: CodeAnalysisResult, filePath: string): void {
