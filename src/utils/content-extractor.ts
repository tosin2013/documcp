...
function categorizeDocument(content: string, filePath: string): ExtractedContent['existingDocs'][0]['category'] {
  const lowerContent = content.toLowerCase();
  const lowerPath = filePath.toLowerCase();
  
  if (lowerPath.includes('tutorial') || lowerPath.includes('getting-started') || 
      lowerContent.includes('## getting started') || lowerContent.includes('# tutorial')) {
    return 'tutorial';
  }
  ...
  if (lowerPath.includes('api') || lowerPath.includes('reference') || 
      lowerContent.includes('## api') || lowerContent.includes('# reference')) {
    return 'reference';
  }
  
  if (lowerPath.includes('concept') || lowerPath.includes('explanation') || 
      lowerPath.includes('architecture') || lowerPath.includes('adr')) {
    return 'explanation';
  }
...
function parseADR(content: string, filename: string): ExtractedContent['adrs'][0] | null {
  ...
  for (const line of lines) {
    if (line.startsWith('# ')) {
      adr.title = line.replace('# ', '').replace(/^\d+\.?\s*/, '').trim();
      break;
    }
  }
...
function parseMarkdownAPI(content: string): ExtractedContent['apiDocs'] {
  ...
  for (const section of sections) {
    if (section.includes('API') || section.includes('endpoint') || section.includes('function')) {
      ...
      // Extract parameters
      const paramsIndex = lines.findIndex(l => l.includes('Parameters') || l.includes('Arguments'));
      if (paramsIndex !== -1) {
