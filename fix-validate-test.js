...
const testFile =
  "/Users/tosinakinosho/workspaces/documcp/tests/tools/validate-readme-checklist.test.ts";
...
const patterns = [
  // Simple readmePath only
  {
    ...
    to: (match, readmePath) =>
      `validateReadmeChecklist(ValidateReadmeChecklistSchema.parse({ readmePath: ${readmePath} }))`,
  },
  // readmePath with projectPath
  {
    from: /validateReadmeChecklist\(\{\s*readmePath:\s*([^,}]+),\s*projectPath:\s*([^}]+)\s*\}\)/g,
    to: (match, readmePath, projectPath) =>
      `validateReadmeChecklist(ValidateReadmeChecklistSchema.parse({ readmePath: ${readmePath}, projectPath: ${projectPath} }))`,
  },
  // readmePath with outputFormat
  {
    from: /validateReadmeChecklist\(\{\s*readmePath:\s*([^,}]+),\s*outputFormat:\s*([^}]+)\s*\}\)/g,
    to: (match, readmePath, outputFormat) =>
      `validateReadmeChecklist(ValidateReadmeChecklistSchema.parse({ readmePath: ${readmePath}, outputFormat: ${outputFormat} }))`,
  },
];
...
content = content.replace(
  "expect(getTldrCheck(noTldrResult)?.passed).toBe(false);",
