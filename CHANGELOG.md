...
- **llm:** the bundled multi-provider LLM client (`src/utils/llm-client.ts`) is removed. Future LLM-driven features will use **MCP Sampling** instead.
- **mcp:** the `simulate_execution` MCP tool is removed; there is no AST-only equivalent. Hosts that called it must drop the call or migrate to MCP Sampling client-side.
- **api:** `createSemanticAnalyzer({ useLLM, confidenceThreshold, llmConfig })` no longer accepts these options; analysis mode is always `'ast'`.
- **env:** `DOCUMCP_LLM_PROVIDER`, `DOCUMCP_LLM_API_KEY`, `DOCUMCP_LLM_MODEL`, and `DOCUMCP_LLM_BASE_URL` are silently ignored in v0.6.0 (with a deprecation warning) and removed entirely in v0.7.0.
- **docs:** `docs/how-to/llm-integration.md` is removed; see migration notes in the release preview.

### 🧹 Removed

- delete `src/utils/llm-client.ts` (369 LOC) — see [#107](https://github.com/tosin2013/documcp/issues/107).
- delete `src/utils/execution-simulator.ts` and `src/tools/simulate-execution.ts` (553 LOC) — see [#109](https://github.com/tosin2013/documcp/issues/109).
- delete `tests/utils/llm-client.test.ts` and the `simulate_execution` integration tests.

### 🔧 Refactored

- refactor: collapse `src/utils/semantic-analyzer.ts` to AST-only mode; remove `useLLM`/`hybrid` branching — see [#108](https://github.com/tosin2013/documcp/issues/108).
- refactor: drop `DOCUMCP_LLM_*` env-var parsing from `src/index.ts` with a one-version deprecation warning.

### 📚 Documentation

- docs: ADR-009 Phase 2.5 marked **Superseded** by [ADR-014](docs/adrs/adr-0014-removal-of-server-side-llm-dependency-in-favor-of-mcp-sampling.md) — see [#110](https://github.com/tosin2013/documcp/issues/110).
- docs: add ADRs 014–018 to [docs/adrs/](docs/adrs/) and update the index.
- docs: add [RELEASE_PLAN.md](RELEASE_PLAN.md) and [docs/development/v0.6.0-release-preview.md](docs/development/v0.6.0-release-preview.md).
- docs: ADR-005 alternative "Third-Party Deployment Services" marked **Revisited by ADR-018**.

### 🔗 Migration

If you set `DOCUMCP_LLM_*` env vars, simply remove them. If you imported `LLMClient` programmatically, vendor the file from the `v0.5.x` tag or migrate to MCP Sampling client-side. If you called the `simulate_execution` MCP tool, remove the call from your host config — the tool no longer exists. Full migration guide: [docs/development/v0.6.0-release-preview.md#migration-guide](docs/development/v0.6.0-release-preview.md#migration-guide).
